...
PGOPTIONS="--statement-timeout=${STATEMENT_TIMEOUT}" psql "${DB_NAME}" -q -c "DROP TRIGGER IF EXISTS check_price ON BOM;" > /dev/null 2>&1
PGOPTIONS="--statement-timeout=${STATEMENT_TIMEOUT}" psql "${DB_NAME}" -q -c "DROP FUNCTION IF EXISTS check_price_function();" > /dev/null 2>&1
...
PGOPTIONS="--statement-timeout=${STATEMENT_TIMEOUT}" psql "${DB_NAME}" -c "INSERT INTO BOM VALUES(11, 1, 1);" > "${OUTPUT_FILENAME}" 2>&1
PGOPTIONS="--statement-timeout=${STATEMENT_TIMEOUT}" psql "${DB_NAME}" -c "INSERT INTO BOM VALUES(12, 1, 200);" >> "${OUTPUT_FILENAME}" 2>&1
PGOPTIONS="--statement-timeout=${STATEMENT_TIMEOUT}" psql "${DB_NAME}" -c "INSERT INTO BOM VALUES(13, 1, 1);" >> "${OUTPUT_FILENAME}" 2>&1
PGOPTIONS="--statement-timeout=${STATEMENT_TIMEOUT}" psql "${DB_NAME}" -c "UPDATE BOM SET Price = 300 WHERE PartID = 13;" >> "${OUTPUT_FILENAME}" 2>&1
